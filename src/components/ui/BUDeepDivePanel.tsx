...
export function BUDeepDivePanel({ buName, detail, onClose, filter }: Props) {
  ...
  // Budget is compared against Formal Training spend only, so bu.budgetUtilisation already
  // matches the "Training Spend" MiniKPI's bu.trainingCost figure this subtitle describes.
          ...
          <section>
            ...
            {(detail.avgVendorRating > 0 || detail.hoursReport.hasData) && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
                ...
                {detail.hoursReport.hasData && (
                  <MiniKPI label={`${detail.hoursReport.hoursThreshold}-Hour Compliance`} value={`${detail.hoursReport.staffMeeting40hPct.toFixed(0)}%`} sub={`${detail.hoursReport.staffMeeting40h} of ${bu.totalStaff || detail.hoursReport.staffMeeting40h + detail.hoursReport.staffBelow40h} staff`} accent={detail.hoursReport.staffMeeting40hPct >= 80 ? 'text-green-700' : detail.hoursReport.staffMeeting40hPct >= 50 ? 'text-amber-700' : 'text-red-700'} alert={detail.hoursReport.staffMeeting40hPct < 50 && detail.hoursReport.hasData} />
                )}
              </div>
            )}
