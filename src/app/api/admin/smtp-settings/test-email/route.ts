...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await sendMail({
      ...
      html: '<p>This is a test email from the Learning Intelligence Dashboard’s SMTP settings. If you received this, SMTP is configured correctly.</p>',
    })
    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('[admin/smtp-settings/test-email]', err)
