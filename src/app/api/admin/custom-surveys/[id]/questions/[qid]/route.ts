...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string; qid: string }> }) {
  ...
  if (survey.status !== 'draft' && survey.status !== 'launched') {
    return NextResponse.json({ error: 'Questions can only be edited while the survey is a draft or currently launched.' }, { status: 400 })
  }
  // Once launched, people may have already answered against the CURRENT type/options/ratingMax/
  // section/branching — changing any of those would silently invalidate or misinterpret responses
  // already collected (an answer recorded against option list A no longer means anything sensible
  // once the question's options become list B), so those stay locked. Wording, an added
  // description, whether it's required (only affects people who haven't answered yet), and the
  // file-upload folder are all safe to change without affecting analysis of what's already in.
  const isLive = survey.status === 'launched'
